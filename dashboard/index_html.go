...
function DistributionRow(name){
  ...
  this.makeToggles = function(){
    var name = this.name
    var row = [
      this.graphToggle("heatmap",
        function(){ return {name: name, window: data().Window} },
        HeatmapUpdater, "lightgreen"),
      this.graphToggle("samples", function(){ return name },
...
function HeatmapUpdater(name, accessor, div){
  ...
  this.preload = function(arr){
    ...
    $.getJSON("/metric",
      ...
      function(d){
        d = d.Value
        var rangeslice = dataRangeSlice(d.Samples, 0.2, 0.8, 2)
...
function Heatmap(div, range){
  ...
  var updateMousemove = function(){
    ...
    str += Math.floor(bin.x + bin.dx).toPrecision(3) + ") "
